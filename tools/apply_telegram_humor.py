...
def clean_product_title(t: str) -> str:
    if not t: return "Produto em promoção"
    res = html.unescape(t)
    res = re.sub(r"&[a-zA-Z0-9#]+;", " ", res)
    res = re.sub(r"\s*:\s*Amazon\.[a-z.]+:?\s*.*$", "", res, flags=re.I)
    res = re.sub(r"\s*por\s+[\d,.]+\s*[\u20ac€].*$", "", res, flags=re.I)
    res = re.sub(r"\s*[\|–-]\s*Amazon\s*.*$", "", res, flags=re.I)
    
    es_to_pt = [
        (r"\bZapatos Hombre\b", "Sapatilhas Homem"),
        (r"\bZapatos Mujer\b", "Sapatilhas Mulher"),
        (r"\bZapatillas\b", "Sapatilhas"),
        (r"\bFunda con MagSafe\b", "Capa com MagSafe"),
        (r"\bFunda\b", "Capa"),
...
def extract_specs_list(title: str):
    t = title.lower()
    specs = []

    # 1. AURICULARES & SOM (Prioridade máxima para evitar falso positivo de smartphone por marca)
    if re.search(r"auricular|headphone|earphone|airpods|earbuds|buds\b|fones|soundcore|coluna|soundbar|bluetooth", t):
    ...
    # 2. CAPAS E ACESSÓRIOS DE SMARTPHONE (Prioridade sobre o smartphone em si)
    elif re.search(r"(?:capa|funda|case|cover)\s+(?:com|para|con|for|de)?.*(?:iphone|galaxy|xiaomi|redmi|poco|pixel|telem|smartphone|air\b)|magsafe.*(?:capa|funda|case)|pel[íi]cula|vidro\s*temperado|protector.*pantalla", t):
        m_model = re.search(r"(iphone\s*[\w\s]+|galaxy\s*s\d+[\w\s]*|xiaomi\s*[\w\s]+|pixel\s*\d+[\w\s]*)", t)
        compat = f"Concebida especificamente para {m_model.group(1).title().strip()} com recortes perfeitos" if m_model else "Ajuste preciso milimétrico para total compatibilidade com o dispositivo"
        specs.append(("Compatibilidade", compat))

        if "magsafe" in t:
            specs.append(("Tecnologia MagSafe", "Ímanes integrados de forte fixação para carregamento sem fios rápido e suporte a acessórios magnéticos"))
        
        specs.append(("Proteção Contra Quedas", "Bordos sobrelevados que protegem o ecrã e as lentes da câmara contra riscos e impactos diretos"))
        
        if re.search(r"silicone|gel|tpu", t):
            mat = "Silicone flexível de toque sedoso com revestimento interior em microfibra macia"
        elif re.search(r"transparente|clear|geada", t):
            mat = "Policarbonato transparente de alta resistência com tratamento anti-amarelecimento"
        elif re.search(r"couro|pele|leather", t):
            mat = "Acabamento premium em pele resistente de toque sofisticado e duradouro"
        else:
            mat = "Materiais duráveis de elevada absorção de choque e toque antiderrapante"
        specs.append(("Material & Toque", mat))
        specs.append(("Acesso & Botões", "Botões integrados táteis e de resposta imediata com acesso total à porta de carregamento"))

    # 3. SMARTPHONES & TABLETS (Dispositivo real)
    elif re.search(r"smartphone|telem[oó]vel|iphone|galaxy\s+s\d|galaxy\s+a\d|redmi\s+note|redmi\s+\d|xiaomi\s+\d|poco|pixel\s+\d|oneplus|realme|honor|tablet|ipad", t):
    ...
    elif re.search(r"port[áa]til|laptop|notebook|macbook|chromebook|computador|ideapad|thinkpad|inspiron|pavilion|zenbook|vivobook|pc\b", t):
        m_cpu = re.search(r"(ryzen\s*\d\s*\w*|core\s*i[3579]\s*[\w-]*|intel\s*n\d+|apple\s*m\d|snapdragon\s*x)", t)
        cpu = f"Processador {m_cpu.group(1).upper()} para arranque rápido e fluidez multitarefa" if m_cpu else "Processador de alto rendimento otimizado para produtividade e eficiência energética"
...
PRODUCT_RULES = [
    (r"(?:capa|funda|case|cover)\s+(?:com|para|con|for|de)?.*(?:iphone|galaxy|xiaomi|redmi|poco|pixel|telem|smartphone|air\b)|magsafe.*(?:capa|funda|case)", [
        ("Proteção essencial para o teu equipamento 🛡️", "Protege o teu telemóvel contra quedas e riscos sem perder a elegância. ✨", "Acessório indispensável para manter o valor e a estética do equipamento como novo."),
        ("Design fino com proteção máxima 📱", "Bordos reforçados para proteger câmara e ecrã de acidentes inesperados. 👌", "Encaixe perfeito com suporte a carregamento MagSafe sem fios."),
    ]),
    (r"auricular|headphone|earphone|airpods|earbuds|buds\b|fones|soundcore", [
    ...
    (r"smartphone|telem[oó]vel|iphone|galaxy\s+s\d|galaxy\s+a\d|redmi\s+note|redmi\s+\d|xiaomi\s+\d|poco\s+|pixel\s+\d", [
    ...
    (r"aspirador|robot.*aspirador|roomba|dreame|roborock|vassoura.*aspirador", [
...
def extract_deal_info(soup, html_text):
    raw_title = ""
    t = soup.find("title")
    if t: raw_title = t.get_text()
    if not raw_title:
        h1 = soup.find("h1")
        raw_title = h1.get_text(strip=True) if h1 else "Produto em promoção"
...
def process_directory(dir_path):
    ...
    for fp in sorted(dir_path.glob("**/*.html")):
        if "index.html" not in fp.name and dir_path.name == "r":
            continue
        try:
            html_content = fp.read_text("utf-8", errors="replace")
            soup = BeautifulSoup(html_content, "html.parser")
            
            title, price, old_price, discount, store = extract_deal_info(soup, html_content)
            
            h1 = soup.find("h1")
            if h1:
                h1_clean = clean_product_title(h1.get_text())
                html_content = re.sub(r'<h1>.*?</h1>', f'<h1>{h1_clean}</h1>', html_content, count=1, flags=re.DOTALL)

            title_tag = soup.find("title")
            if title_tag:
                t_clean = clean_product_title(title_tag.get_text())
                html_content = re.sub(r'<title>.*?</title>', f'<title>{t_clean}</title>', html_content, count=1, flags=re.DOTALL)

            new_editorial = build_telegram_style_editorial(title, price, old_price, discount, store)
            
            if "editorial-desc" in html_content:
                html_content = re.sub(
            ...
            elif 'class="editorial"' in html_content:
                html_content = re.sub(
            ...
            fp.write_text(html_content, "utf-8")
            count += 1
...
print("=== REFINAMENTO DE CATEGORIAS, ACESSÓRIOS E FONES ===")
d_count = process_directory(DEALS_DIR)
r_count = process_directory(REDIRECTS_DIR)
print(f"✅ Concluído: {d_count} páginas em /d/ e {r_count} páginas em /r/ com categorias e especificações precisas!")
